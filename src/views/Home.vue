    ...
    <div v-if="discussions.length > 0" id="discussionCarousel" class="carousel slide" data-bs-ride="carousel">
      <div class="carousel-inner">
        ...
        <div v-for="(discussion, index) in discussions.slice(0, 3)"
          ...
          <div class="d-flex justify-content-center align-items-center bg-light text-dark p-4" style="height: 300px;">
            <!-- Display discussion title and description -->
            <div class="text-center">
              <h5>{{ discussion.titre || 'Untitled Discussion' }}</h5>
              <p>{{ discussion.description || 'No description available' }}</p>
              <small>Created by: {{ UserNames[discussion.createdBy] || 'Unknown' }}</small>
            </div>
          </div>
        </div>
      </div>
